...
function ulx.getVersion() -- This exists on the client as well, so feel free to use it!
	...
	if ulx.release then
		...
		if file.Exists( "addons/ulx/.svn/wc.db", true ) then -- SVN's new format
			...
			r = highest]]
		elseif file.Exists( "addons/ulx/lua/ulx/.svn/entries", true ) then
			-- Garry broke the following around 05/11/2010, then fixed it again around 11/10/2010!
			local lines = string.Explode( "\n", file.Read( "lua/ulx/.svn/entries", true ) )
			r = tonumber( lines[ 4 ] )
		end
