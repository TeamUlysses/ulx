...
local utime_outsidetext_r = CreateClientConVar( "utime_outsidetext_r", "256.0", true, false ) 
local utime_outsidetext_g = CreateClientConVar( "utime_outsidetext_g", "256.0", true, false ) 
local utime_outsidetext_b = CreateClientConVar( "utime_outsidetext_b", "256.0", true, false ) 
...
local utime_insidetext_r = CreateClientConVar( "utime_insidetext_r", "256.0", true, false ) 
local utime_insidetext_g = CreateClientConVar( "utime_insidetext_g", "256.0", true, false ) 
local utime_insidetext_b = CreateClientConVar( "utime_insidetext_b", "256.0", true, false )
...
function think()
	...
	gpanel.lblTotalTime:SetFGColor( textColor )
	gpanel.lblSessionTime:SetFGColor( textColor )	
	gpanel.total:SetFGColor( textColor )
	gpanel.session:SetFGColor( textColor )	
	
	local insideTextColor = Color( utime_insidetext_r:GetInt(), utime_insidetext_g:GetInt(), utime_insidetext_b:GetInt(), 255 )	
	gpanel.playerInfo.lblTotalTime:SetFGColor( insideTextColor )
	gpanel.playerInfo.lblSessionTime:SetFGColor( insideTextColor )
	gpanel.playerInfo.lblNick:SetFGColor( insideTextColor )
	gpanel.playerInfo.total:SetFGColor( insideTextColor )
	gpanel.playerInfo.session:SetFGColor( insideTextColor )
	gpanel.playerInfo.nick:SetFGColor( insideTextColor )
end
...
function PANEL:Init()
	...
	self.lblTotalTime	= vgui.Create( "Label", self )
	self.lblSessionTime	= vgui.Create( "Label", self )
	
	self.total			= vgui.Create( "Label", self )
	self.session		= vgui.Create( "Label", self )	
end
...
function PANEL:ApplySchemeSettings()
	self.lblTotalTime:SetFont( "Default" )
	self.lblSessionTime:SetFont( "Default" )
	self.total:SetFont( "Default" )
	self.session:SetFont( "Default" )	
	
	self.lblTotalTime:SetFGColor( color_white )
	self.lblSessionTime:SetFGColor( color_white )
	self.total:SetFGColor( color_white )
	self.session:SetFGColor( color_white )	
end
...
function INFOPANEL:Init()
	self.lblTotalTime	= vgui.Create( "Label", self )
	self.lblSessionTime	= vgui.Create( "Label", self )
	self.lblNick		= vgui.Create( "Label", self )
...
function INFOPANEL:ApplySchemeSettings()
	self.lblTotalTime:SetFont( "Default" )
	self.lblSessionTime:SetFont( "Default" )
	self.lblNick:SetFont( "Default" )
	self.total:SetFont( "Default" )
	self.session:SetFont( "Default" )	
	self.nick:SetFont( "Default" )	
	
	self.lblTotalTime:SetFGColor( color_black )
	self.lblSessionTime:SetFGColor( color_black )
	self.lblNick:SetFGColor( color_black )
	self.total:SetFGColor( color_black )
	self.session:SetFGColor( color_black )	
	self.nick:SetFGColor( color_black )	
end
